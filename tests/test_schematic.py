...
class Tests_Private_Schematics(unittest.TestCase):
    ...
    def test_All_Private(self):
        ...
        if not private_path:
            self.skipTest("Environment variable PRIVATE_KICAD_REPO not set, skipping private schematics test.")

        private_schematics_path = Path(private_path)
        if not private_schematics_path.exists():
            self.skipTest(f"Path {private_schematics_path} does not exist, skipping private schematics test.")
        ...
        if failures:
            failure_messages = "\n".join([f"Schematic: {file}, Error: {error}" for file, error in failures])
